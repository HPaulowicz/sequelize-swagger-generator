...
const constructNameExpression = (type, field, options, applications) => {
    ...
    if (simpleNameExpressions.includes(type.name)) {
        ...
        for (const o in options) {
            if (optionFormatsByType[type.name].includes(o)) {
                typeOptions[o] = options[o];
            }
        }
        console.log('PPPOADJOHDAJDBJODBOABCOJ', type.name, constructOptions(typeOptions));
        // console.log(prop.name, options, typeOptions);
        return prop;
    }
...
const constructObject = (type, field, options, applications) => {
    ...
    if (!(typeof applications === 'undefined')) {
        const properties = applications.reduce((accumulator, application) => {
            const property = constructPropertyType(application, field, options);
            const [ model, p] = application.name.split('.');
            if (p) {
                accumulator[p] = property;
            } else {
                accumulator.allOf = accumulator.allOf ? { ...property } : { ...property, ...accumulator.allOf };
            }
            return accumulator;
        }, {});
...
const constructPropertyType = (type, field = undefined, options = undefined, applications = undefined) => {
    console.log(`\x1b[32m(${field} - DEBUG: ) \x1b[31m%s\x1b[0m`, JSON.stringify(options, null, 2))
...
const constructProperty = (param) => {
    ...
    return {
        property: {[field]: constructPropertyType(type, field, optionsReducer(description))},
